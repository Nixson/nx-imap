...
func (m *Message) Fetch(seqNum uint32, items []imap.FetchItem, changedSince uint32) (*imap.Message, error) {
	...
	for _, item := range items {
		...
		case imap.FetchUid:
			fetched.Uid = m.Uid
		case imap.FetchChangedSince:
			fetched.ChangedSince = changedSince
		default:
