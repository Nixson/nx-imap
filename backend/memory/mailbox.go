...
import (
	...
	"github.com/nixson/nx-imap/backend"
	"github.com/nixson/nx-imap/backend/backendutil"
)
...
func (mbox *Mailbox) ListMessages(uid bool, seqSet *imap.SeqSet, items []imap.FetchItem, ch chan<- *imap.Message, changedSince uint32) error {
	defer close(ch)
	...
	for i, msg := range mbox.Messages {
		...
		m, err := msg.Fetch(seqNum, items, changedSince)
		if err != nil {
			continue
		}
